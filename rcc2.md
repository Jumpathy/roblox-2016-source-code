...
Client GETs `/Game/PlaceLauncher.ashx?placeId=X`. You allocate a UDP port, mint a `jobId`, and send RCCService an `OpenJob` SOAP request. Respond with JSON including `status: 2` and the join/auth URLs.
RCCService loads `gameserver.txt` inside a sandboxed Lua instance and starts listening on the UDP port you gave it. That Lua calls `game:Load(baseUrl .. "/asset/?id=" .. placeId)` — so `/asset/?id=<placeId>` must return the real `.rbxl` file or the instance comes up empty.
Client GETs `/game/authenticate.ashx` to validate and rotate its ticket.
Client GETs `/game/join.ashx?jobId=&port=&placeId=` and receives the join configuration blob (a JSON object with a required leading `\n`).
Client opens a direct UDP connection to `MachineAddress:ServerPort` from the join blob — that's the actual game connection to RCCService.
Your renewal loop fires `RenewLease` every ~60s per active job. If the 300s lease lapses, RCCService kills the instance.
Throughout all of this, the client fires dozens of side-calls — analytics, version checks, feature flags, counters. These all need a `200` so the client doesn't stall. See the stub table at the bottom.
